...
export function ChatDock() {
  ...
  const send = async (text: string) => {
    ...
    setInput("");
    setTyping(true);
    const fallback = localAnswer(question, state, yearlyPay, monthsRecorded);
    try {
      const context = buildChatContext(state, yearlyPay, monthsRecorded);
              ...
              >
                ...
                {m.action ? (
                  <Link
                    ...
                    className="mt-3 inline-flex rounded-full bg-info-soft px-3 py-1.5 text-xs font-semibold text-info-foreground"
                  >
                    {m.action.label} →
                  </Link>
                ) : null}
