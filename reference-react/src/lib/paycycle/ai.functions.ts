...
export const extractDocumentFields = createServerFn({ method: "POST" })
  .inputValidator((input: { kind: DocKind; dataUrl: string; period: string }) => input)
  .handler(async ({ data }) => {
...
export const translateMessage = createServerFn({ method: "POST" })
  .inputValidator((input: { korean: string; languageName: string }) => input)
  .handler(async ({ data }) => {
    const result = await callGateway([
      {
        role: "system",
...
export const answerWithContext = createServerFn({ method: "POST" })
  .inputValidator((input: { question: string; context: string }) => input)
  .handler(async ({ data }) => {
    const result = await callGateway([
      {
        role: "system",
