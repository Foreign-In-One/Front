...
> = {
  ...
  PAYCHECK: {
    ...
    chipStyle:
      'bg-primary/20 text-primary dark:text-primary-foreground shadow-xs',
    icon: Landmark,
  },
  TAX: {
...
export default function CalendarPage() {
  ...
  const allEvents = useMemo(() => {
    ...
    if (state.employment?.payDay) {
      ...
      const hasPayDayOnDate = list.some(
        (e) => e.date === payIso && e.type === 'PAYDAY',
      );
      if (!hasPayDayOnDate) {
        list.push({
    ...
    // 3. 예상 출국일 이벤트 (중복 방지)
    if (
      state.employment?.exitDate?.value &&
      !state.employment.exitDate.unknown
    ) {
      const hasExitOnDate = list.some(
        (e) =>
          e.date === state.employment?.exitDate?.value && e.type === 'EXIT',
      );
      if (!hasExitOnDate) {
        list.push({
  ...
  return (
    <AppShell title={t('cal.title')} subtitle={t('cal.subtitle')}>
      {/* 1. 달력 상단 헤더 */}
      <div className="flex items-center justify-between rounded-3xl border border-border/70 bg-card p-5 shadow-xs backdrop-blur-md">
        <Button
          variant="ghost"
          size="icon"
          onClick={prevMonth}
          className="rounded-2xl border border-border/60 bg-background shadow-xs"
        >
      ...
      {/* 2. 7열 달력 그리드 */}
      <div className="mt-5 space-y-2 rounded-3xl border border-border/70 bg-card p-5 shadow-xs backdrop-blur-md">
        {/* 요일 헤더 */}
        <div className="grid grid-cols-7 border-border/40 border-b pb-3 text-center">
          {weekdays.map((w, idx) => (
            <span
              key={w}
          ...
          {calendarDays.slice(0, 35).map((d) => {
            ...
            return (
              <button
                ...
                className={`relative flex min-h-[4.4rem] flex-col justify-between overflow-hidden rounded-2xl border p-2.5 text-xs transition-all ${
                  !d.isCurrentMonth
                    ? 'border-transparent opacity-30'
                    : 'opacity-100'
                } ${
                  isSelected
                    ? 'z-10 border-primary bg-primary font-bold text-primary-foreground'
                    : isToday
                      ? 'border-primary/30 bg-primary/10 font-bold text-primary shadow-xs'
                      : 'border-border/50 bg-background/80 font-semibold text-foreground shadow-xs hover:bg-muted/80'
                }`}
              >
                <div className="flex w-full items-center justify-between">
                  <span
                    className={`font-bold text-[11px] ${isSelected ? 'text-primary-foreground' : ''}`}
                  >
                    {d.dayNum}
                  </span>
                  {dayEvents.length > 2 && (
                    <span
                      className={`rounded-full px-1.5 font-bold text-[9px] ${isSelected ? 'bg-primary-foreground/25 text-primary-foreground' : 'bg-primary/15 text-primary shadow-2xs'}`}
                    >
                      +{dayEvents.length}
                    </span>
                  )}
                ...
                <div className="mt-1 w-full space-y-0.5">
                  {dayEvents.slice(0, 2).map((evt) => {
                    ...
                    return (
                      <div
                        key={evt.id}
                        className={`truncate rounded-md px-1.5 py-0.5 text-left font-bold text-[9px] leading-tight transition-all ${
                          isSelected ? 'bg-white/20 text-white' : meta.chipStyle
                        }`}
                      >
                        {t(meta.labelKey)}
                      </div>
      ...
      <div className="pc-rise mt-6 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Sparkles className="size-4 text-primary" />
            <h3 className="font-bold text-foreground text-sm">
              {t('cal.dayEventsCount', {
                date: formatKDate(selectedDate),
        ...
        {selectedDayEvents.length === 0 ? (
          <div className="rounded-3xl border border-border/60 bg-card p-8 text-center font-semibold text-muted-foreground text-xs shadow-xs backdrop-blur-md">
          ...
          selectedDayEvents.map((evt) => {
            ...
            return (
              <div
                key={evt.id}
                className={`flex items-start justify-between rounded-3xl p-5 shadow-xs backdrop-blur-md ${meta.tone} ${
              ...
              >
                <div className="flex items-start gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      {evt.time && (
                        <span className="inline-flex items-center gap-1 rounded-full bg-card/90 px-2.5 py-0.5 font-bold text-[10px] text-foreground shadow-xs">
                          <Clock className="size-3 text-primary" />
                          {evt.time}
                        </span>
                      )}
                      <span className="rounded-full bg-card/90 px-2.5 py-0.5 font-bold text-[10px] text-foreground shadow-xs">
                        {t(meta.labelKey)}
                      </span>
                    </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="z-[100] rounded-3xl border border-border bg-card p-6 text-card-foreground shadow-2xl sm:max-w-md">
          ...
          <div className="space-y-4 pt-2">
            <div>
              <label
                htmlFor="cal-date-input"
                className="mb-1 block font-bold text-foreground text-xs"
              >
                {t('cal.date')}
              </label>
              <Input
                ...
                className="mt-1 rounded-2xl border border-input bg-background font-bold text-foreground text-xs shadow-xs focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>

            <div>
              <label
                htmlFor="cal-time-input"
                className="mb-1 block font-bold text-foreground text-xs"
              >
                {t('cal.time')}
              </label>
            ...
            <div>
              <label
                htmlFor="cal-event-type-select"
                className="mb-1 block font-bold text-foreground text-xs"
              >
                {t('cal.eventType')}
              </label>
              <select
                id="cal-event-type-select"
                value={newType}
                onChange={(e) => setNewType(e.target.value as EventType)}
                className="mt-1 w-full rounded-2xl border border-input bg-background p-3.5 font-bold text-foreground text-xs shadow-xs focus:ring-2 focus:ring-ring"
              >
            ...
            <div>
              ...
              <Input
                ...
                className="mt-1 rounded-2xl border border-input bg-background font-bold text-foreground text-xs shadow-xs placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>

            <div>
              <label
                htmlFor="cal-event-desc-input"
                className="mb-1 block font-bold text-foreground text-xs"
              >
                {t('cal.eventDesc')}
              </label>
              <Input
                id="cal-event-desc-input"
            ...
            <div className="flex justify-end gap-2 pt-3">
              <Button
                variant="ghost"
                className="rounded-2xl font-bold text-muted-foreground text-xs hover:bg-muted hover:text-foreground"
